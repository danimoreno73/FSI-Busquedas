...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print("Expanded nodes: " + f'{generated_nodes}'
                  + "\nVisited nodes: " + f'{visited_nodes}'
                  + "\nCost: " + f'{node.path_cost}')
            return node
        if node.state not in closed:
            closed[node.state] = True
            size = fringe.__len__()
            fringe.extend(node.expand(problem))
            size = fringe.__len__() - size
...
def breadth_first_graph_search(problem):
    ...
    search = graph_search(problem, FIFOQueue())  # FIFOQueue -> fringe
...
def depth_first_graph_search(problem):
    ...
    search = graph_search(problem, Stack())  # FIFOQueue -> fringe
...
def branch_and_bound_search(problem):
    search = graph_search(problem, BaBList())
